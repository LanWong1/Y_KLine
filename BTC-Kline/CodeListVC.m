...
- (void)addActiveId{
    ...
    self.activeId.center = CGPointMake(self.view.center.x ,self.view.center.y+200);
    [self.view addSubview:self.activeId];
}
//"please wait"
- (void)addLabel{
    self.label = [[UILabel alloc]initWithFrame:CGRectMake(self.view.center.x-40, self.view.center.y-200, 80, 20)];
...
- (void)addSearch{
    ...
    self.searchView = [[UIView alloc]initWithFrame:CGRectMake(0, 55, [UIScreen mainScreen].bounds.size.width, self.searchController.searchBar.frame.size.height)];
...
- (void)addTableView{
    ...
    [self.tableView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.equalTo(@([UIScreen mainScreen].bounds.size.height-120));
    }];
}
...
- (void)addSementView{
    ...
    self.segment.frame = CGRectMake(0, 58, [UIScreen mainScreen].bounds.size.width, 50);
